...
Evidence for C3 would include code found on lines 155-159 of the handle._.c file and code regarding topic length found on lines 134-145. The Misquitto configuration restricts messages/files to a predetermined length and prohibits files from exceeding these requirements. The combined code evidence cited above, collectively, protects the broker against buffer overflow attacks. Lastly, evidence for C4 would require an additional capability by Misquitto, in a future-state, that would limit the data file types allowed by the broker thereby offering an added layer of security against malware attacks. 
